...
def process_masterlist_merger(master_path, parcel_path, output_dir):
    """
    MODE 2: TRIAGE SYSTEM (Smart Date Logic)
    
    Updates:
    - Birthday Check: Allows a 5-day margin of error.
    - Integrity Check: Restricted to Bio-data (Gender, Birthday, Sector).
    - Output: Keeps Masterlist columns as source of truth.
    """
    ...
    # --- INTEGRITY MAP ---
    # Disabled Names/Addresses to prevent false positives.
    INTEGRITY_MAP = {
        ...
        # 'last_name': 'LAST NAME',
        # 'ext_name': 'EXT NAME',
        # 'farmer_address_bgy': 'FARMER ADDRESS 1',
        # 'farmer_address_mun': 'FARMER ADDRESS 2',
        # 'farmer_address_prv': 'FARMER ADDRESS 3',
        'birthday': 'BIRTHDATE',
        'gender': 'GENDER',
        'farmer': 'FARMER',
    ...
    try:
        ...
        # --- 4. MERGE & INTEGRITY CHECK (Smart Date) ---
        with LoadingSpinner("Merging & Validating Integrity..."):
            ...
            for idx, row in df_merged[mask_has_parcel].iterrows():
                ...
                for m_col, p_col_name in INTEGRITY_MAP.items():
                    ...
                    if actual_p_col in df_m.columns: target_p_col = f"{actual_p_col}_PARCEL"
                    ...
                    if not is_empty_m and not is_empty_p:
                        ...
                        # Special Logic: BIRTHDAY (5-day Margin)
                        if m_col == 'birthday':
                            try:
                                d_m = pd.to_datetime(val_m)
                                d_p = pd.to_datetime(val_p)
                                # Check absolute difference in days
                                if abs((d_m - d_p).days) <= 5:
                                    match_found = True
                            except:
                                # Fallback if dates are invalid strings
                                pass
                        
                        # Default Logic: Strict Equality
                        if not match_found:
                            if val_m == val_p:
                                match_found = True
        ...
        with LoadingSpinner("Finalizing Report..."):
            ...
            # Identify columns to drop (Redundant Parcel Data)
            ...
            for target in cols_to_drop_targets:
                ...
                if lower_target in p_cols_map:
                    ...
                    cols_to_remove.append(real_name)
                    cols_to_remove.append(f"{real_name}_PARCEL")
            ...
            with pd.ExcelWriter(output_path, engine='xlsxwriter') as writer:
                ...
                # No Parcels (Filter relevant cols)
        ...
        if integrity_errors:
            print(f"   ⚠️ Found {len(integrity_errors)} Mismatches (5-day Margin applied).")
